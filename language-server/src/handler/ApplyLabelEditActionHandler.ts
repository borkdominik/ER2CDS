...
export class ApplyLabelEditActionHandler {
            ...
            if (!type) {
                model.entities.forEach((e) => {
                    if (e.name === element && ((e.$cstNode) as CompositeCstNodeImpl).content[1].range)
                        server.dispatch(this.createWorkspaceEditReplaceAction(server, sourceUri, ((e.$cstNode) as CompositeCstNodeImpl).content[1].range, action.text));
                });

                model.relationships.forEach((r) => {
                    if (r.name === element && ((r.$cstNode) as CompositeCstNodeImpl).content[1].range)
                        server.dispatch(this.createWorkspaceEditReplaceAction(server, sourceUri, ((r.$cstNode) as CompositeCstNodeImpl).content[1].range, action.text));
                });
                ...
                model.entities.filter(e => e.name === element).forEach(e => e.attributes.forEach((a) => {
                    if (a.name === name) {
                        if (type === 'name' && ((a.$cstNode) as CompositeCstNodeImpl).content[0].range)
                            server.dispatch(this.createWorkspaceEditReplaceAction(server, sourceUri, ((a.$cstNode) as CompositeCstNodeImpl).content[0].range, action.text));

                        if (type === 'datatype' && a.datatype?.$cstNode?.range)
                            server.dispatch(this.createWorkspaceEditReplaceAction(server, sourceUri, a.datatype?.$cstNode?.range, action.text));
                    }
                }));
            }
        }
    ...
    private createWorkspaceEditReplaceAction(server: ER2CDSDiagramServer, sourceUri: URI, range: Range, text: string): WorkspaceEditAction {
